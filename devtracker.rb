...
get '/search/?' do
	...
	dfidCountriesResults.each do |results|
		...
		dfidCountryBudgets[results["code"]] = {}
		dfidCountryBudgets[results["code"]][0] = 0
		dfidCountryBudgets[results["code"]][1] = results["name"]
	end
	dfidRegionsResults.each do |results|
		recipient_regions.concat(results["code"] + ',');
		dfidRegionBudgets[results["code"]] = {}
		dfidRegionBudgets[results["code"]][0] = 0
		dfidRegionBudgets[results["code"]][1] = results["name"]
	end
	oipa_total_project_budget = RestClient.get settings.oipa_api_url + "activities/aggregations?format=json&reporting_organisation=GB-1&budget_period_start=#{settings.current_first_day_of_financial_year}&budget_period_end=#{settings.current_last_day_of_financial_year}&group_by=recipient_country&aggregations=budget&recipient_country="+recipient_countries
	countries_project_budget = JSON.parse_nil(oipa_total_project_budget)
	unless dfidCountryBudgets.empty?
		countries_project_budget['results'].each do |budgets|
			unless budgets['recipient_country']['code'].nil?
				dfidCountryBudgets[budgets['recipient_country']['code']][0] = Money.new(budgets['budget'].to_f*100,"GBP").format(:no_cents_if_whole => true,:sign_before_symbol => false)
			end
		end
	end
	...
	unless dfidRegionBudgets.empty?
		regions_project_budget['results'].each do |budgets|
			unless budgets['recipient_region']['code'].nil?
				dfidRegionBudgets[budgets['recipient_region']['code']][0] = Money.new(budgets['budget'].to_f*100,"GBP").format(:no_cents_if_whole => true,:sign_before_symbol => false)
			end
		end
	end
	...
	:locals => {
		...
		budgetHigherBound: project_budget_higher_bound,
		highLevelSectorList: highLevelSectorList,
		dfidCountryBudgets: dfidCountryBudgets,
		dfidRegionBudgets: dfidRegionBudgets
	}
end
