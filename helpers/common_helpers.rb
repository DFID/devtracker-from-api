...
module CommonHelpers
  ...
  def generateCountryData()
    ...
    sectorBudgets = Oj.load(RestClient.get  api_simple_log(settings.oipa_api_url_solr + 'budget?q=participating_org_ref:GB-* AND reporting_org_ref:('+settings.goverment_department_ids.gsub(","," OR ")+') AND budget_period_start_iso_date:['+current_first_day_of_financial_year.to_s+'T00:00:00Z TO *] AND budget_period_end_iso_date:[* TO '+current_last_day_of_financial_year.to_s+'T00:00:00Z]&json.facet={"items":{"type":"terms","field":"recipient_country_code","limit":-1,"sort":"value asc","facet":{"value":"sum(budget_value)","name":{"type":"terms","field":"recipient_country_name","limit":1},"sectors":{"type":"terms","field":"sector_code","limit":-1,"facet":{"value":"sum(budget_value)"}},"region":{"type":"terms","field":"recipient_region_code","limit":1}}}}&rows=0'))
