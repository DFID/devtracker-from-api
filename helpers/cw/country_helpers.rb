module CountryHelpersCW

  include CommonHelpers
  
  def get_top_5_countries_cw()

      firstDayOfFinYear = first_day_of_financial_year(DateTime.now)
      lastDayOfFinYear = last_day_of_financial_year(DateTime.now)

      countriesInfo = JSON.parse(File.read('data/cw/countries.json'))

      top5countriesJSON = RestClient.get settings.oipa_api_url + "budgets/aggregations/?reporting_organisation=GB-GOV-1&group_by=recipient_country&aggregations=value&budget_period_start=#{firstDayOfFinYear}&budget_period_end=#{lastDayOfFinYear}&order_by=-value&page_size=10&format=json&recipient_country=CM,GH,KE,LS,MW,MZ,NG,RW,SL,ZA,UG,TZ,ZM,IN,BD,PK,BS,DM,JM,VU,LK"
  ...
  def get_country_code_name_cw(countryCode)
      countriesInfo = Oj.load(File.read('data/cw/countries.json'))
  ...
  def get_country_details_cw(countryCode)
      ...
      countryOperationalBudgetInfo = Oj.load(File.read('data/cw/countries_operational_budgets.json'))
  ...
  def get_country_results_cw(countryCode)
      resultsInfo = Oj.load(File.read('data/cw/results.json'))
      results = resultsInfo.select {|result| result['code'] == countryCode}
  end


  def get_country_or_region_cw(projectId)
      #get the data
  ...
  def get_country_region_yearwise_budget_graph_data_cw(apiLink)
  ...
  def get_country_sector_graph_data_cw(countrySpecificsectorValuesJSONLink)
  ...
  def get_country_all_projects_data_cw(countryCode)
    allProjectsData = {}
    ...
    participatingOrgInfo = JSON.parse(File.read('data/cw/participatingOrgList.json'))
  ...
  def get_country_all_projects_data_para_cw(countryCode)
    allProjectsData = {}
  ...
  def get_country_all_projects_rss_cw(countryCode)
  ...
  def total_country_budget_location_cw
    firstDayOfFinYear = first_day_of_financial_year(DateTime.now)
    lastDayOfFinYear = last_day_of_financial_year(DateTime.now)
    ...
    #oipa 3.1
    totalCountryBudgetLocation = RestClient.get settings.oipa_api_url + "budgets/aggregations/?reporting_organisation=GB-GOV-1&recipient_country=CM,GH,KE,LS,MW,MZ,NG,RW,SL,ZA,UG,TZ,ZM,IN,BD,PK,BS,DM,JM,VU,LK&group_by=recipient_country&aggregations=value&budget_period_start=#{firstDayOfFinYear}&budget_period_end=#{lastDayOfFinYear}&format=json&order_by=-value"
  ...
  def pick_top_six_results_cw(countryCode)
    filteredResults = get_country_results(countryCode);
    top6ResultsTitles = JSON.parse(File.read('data/cw/top6ResultsTitles.json'))
    tempHash = []
