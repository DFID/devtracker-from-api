...
=end
	...
	def generate_searched_data_cw(query,activityStatusList)
		...
		searchedData = {}
		dfidCountriesInfo = JSON.parse(File.read('data/cw/dfidCountries.json')) # Stores all the DFID relevant country codes and names by parsing the relevant json file
		dfidRegionsInfo = JSON.parse(File.read('data/cw/dfidRegions.json')) # Stores all the DFID relevant region codes and names by parsing the relevant json file
		...
		begin	
			searchedData['actualStartDate'] = RestClient.get settings.oipa_api_url + "activities/?format=json&page_size=1&fields=activity_dates&hierarchy=1&q=#{query}&ordering=actual_start_date&start_date_gte=1900-01-02&activity_status="+activityStatusList+'&recipient_country=CM,GH,KE,LS,MW,MZ,NG,RW,SL,ZA,UG,TZ,ZM,IN,BD,PK,BS,DM,JM,VU,LK'
		...
		begin	
			searchedData['plannedEndDate'] = RestClient.get settings.oipa_api_url + "activities/?format=json&page_size=1&fields=activity_dates&hierarchy=1&q=#{query}&ordering=-planned_end_date&end_date_isnull=False&activity_status="+activityStatusList+'&recipient_country=CM,GH,KE,LS,MW,MZ,NG,RW,SL,ZA,UG,TZ,ZM,IN,BD,PK,BS,DM,JM,VU,LK'
		...
		oipa_document_type_list = RestClient.get settings.oipa_api_url + "activities/aggregations/?format=json&group_by=document_link_category&aggregations=count&reporting_organisation=GB-GOV-1&q=#{query}&activity_status="+activityStatusList+'&recipient_country=CM,GH,KE,LS,MW,MZ,NG,RW,SL,ZA,UG,TZ,ZM,IN,BD,PK,BS,DM,JM,VU,LK'
	...
	def get_static_filter_list_cw()
		staticFilterList = Oj.load(File.read('data/cw/countryProjectsFilters.json'))
	end
