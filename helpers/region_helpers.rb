...
module RegionHelpers
  ...
  def get_region_detailsv2(regionCode)
    regionInfo = JSON.parse(File.read('data/dfidRegions.json'))
    region = regionInfo.select {|region| region['code'] == regionCode}.first
    totalProjectCount = JSON.parse(RestClient.get api_simple_log(settings.oipa_api_url + "activity/?q=recipient_region_code:#{regionCode} AND reporting_org_ref:GB-GOV-* AND hierarchy:1&fl=iati_identifier&start=0&rows=1"))['response']['numFound']
    totalActiveProjects = JSON.parse(RestClient.get api_simple_log(settings.oipa_api_url + "activity/?q=recipient_region_code:#{regionCode} AND reporting_org_ref:GB-GOV-* AND activity_status_code:2 AND hierarchy:1&fl=iati_identifier&start=0&rows=1"))['response']['numFound']
    allRelatedctivities = JSON.parse(RestClient.get api_simple_log(settings.oipa_api_url + "activity/?q=recipient_region_code:#{regionCode} AND reporting_org_ref:GB-GOV-* AND activity_status_code:2 AND hierarchy:1&fl=related_budget_period_end_quarter,related_budget_period_start_quarter,recipient_region_code,recipient_region_percentage,activity_plus_child_aggregation_budget_value_gbp,related_budget_period_start_iso_date,related_budget_period_end_iso_date,related_budget_value&start=0&rows=1"))['response']['docs']
    totalRegionBudget = 0
  ...
  #   end
    ...
    def get_region_sector_graph_data_jsCompatibleV2(regionCode)
      ...
      api = RestClient.get api_simple_log(settings.oipa_api_url + "activity/?q=recipient_region_code:#{regionCode} AND reporting_org_ref:GB-GOV-*&fl=sector*,iati_identifier,child_aggregation_budget_value_gbp&start=0&rows=1000")
