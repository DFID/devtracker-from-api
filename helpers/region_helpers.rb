...
module RegionHelpers
  ...
  def get_region_details(regionCode)
      ...
      currentTotalRegionBudget= get_current_total_budget(RestClient.get  api_simple_log(settings.oipa_api_url_solr + 'budget?q=participating_org_ref:GB-* AND reporting_org_ref:('+settings.goverment_department_ids.gsub(","," OR ")+') AND budget_period_start_iso_date:['+firstDayOfFinYear.to_s+'T00:00:00Z TO *] AND budget_period_end_iso_date:[* TO '+lastDayOfFinYear.to_s+'T00:00:00Z] AND recipient_region_code:('+regionCode+')&json.facet={"items":{"type":"terms","field":"recipient_region_code","limit":-1,"sort":"value desc","facet":{"value":"sum(budget_value)"}}}&rows=0'))
      ...
      currentTotalDFIDBudget = get_current_dfid_total_budget(RestClient.get  api_simple_log(settings.oipa_api_url_solr + 'budget?q=participating_org_ref:GB-* AND reporting_org_ref:('+settings.goverment_department_ids.gsub(","," OR ")+') AND budget_period_start_iso_date:['+firstDayOfFinYear.to_s+'T00:00:00Z TO *] AND budget_period_end_iso_date:[* TO '+lastDayOfFinYear.to_s+'T00:00:00Z] AND recipient_region_code:('+regionCode+')&json.facet={"items":{"type":"terms","field":"reporting_org_ref","limit":-1,"sort":"value desc","facet":{"value":"sum(budget_value)"}}}&rows=0'))
