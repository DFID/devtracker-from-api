...
=end
  ...
  def get_ogd_all_projects_data_json(ogdCode,projectStatus)
    allProjectsData = {}
    allProjectsData['countryAllProjectFilters'] = get_static_filter_list()
    ...
    allProjectsData['actualStartDate'] = RestClient.get settings.oipa_api_url + "activities?format=json&page_size=1&fields=activity_dates&reporting_organisation=#{ogdCode}&hierarchy=1&ordering=actual_start_date&start_date_gte=1900-01-02&activity_status=#{projectStatus}"
    allProjectsData['actualStartDate'] = JSON.parse(allProjectsData['actualStartDate'])
    if(allProjectsData['actualStartDate']['count'] > 0)
        tempStartDate = allProjectsData['actualStartDate']['results'][0]['activity_dates'].select{|activityDate| activityDate['type']['code'] == '2'}.first
        if (tempStartDate.nil?)
    ...
    allProjectsData['plannedEndDate'] = RestClient.get settings.oipa_api_url + "activities?format=json&page_size=1&fields=activity_dates&reporting_organisation=#{ogdCode}&hierarchy=1&ordering=-planned_end_date&end_date_isnull=False&activity_status=#{projectStatus}"
    allProjectsData['plannedEndDate'] = JSON.parse(allProjectsData['plannedEndDate'])
    if(allProjectsData['plannedEndDate']['count'] > 0)
        tempEndDate = allProjectsData['plannedEndDate']['results'][0]['activity_dates'].select{|activityDate| activityDate['type']['code'] == '3'}.first
        if (tempEndDate.nil?)
    ...
    oipa_document_type_list = RestClient.get settings.oipa_api_url + "activities/aggregations/?format=json&group_by=document_link_category&aggregations=count&reporting_organisation=#{ogdCode}&activity_status=#{projectStatus}"
    document_type_list = JSON.parse(oipa_document_type_list)
    allProjectsData['document_types'] = document_type_list['results']

    #Implementing org type filters
    participatingOrgInfo = JSON.parse(File.read('data/participatingOrgList.json'))
    oipa_implementingOrg_type_list = RestClient.get settings.oipa_api_url + "activities/aggregations/?format=json&group_by=participating_organisation&aggregations=count&reporting_organisation=#{ogdCode}&hierarchy=1&activity_status=#{projectStatus}"
    implementingOrg_type_list = JSON.parse(oipa_implementingOrg_type_list)
    allProjectsData['implementingOrg_types'] = implementingOrg_type_list['results']
    allProjectsData['implementingOrg_types'].each do |implementingOrgs|
      if implementingOrgs['participating_organisation'].length < 1
