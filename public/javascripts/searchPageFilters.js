...
$(document).ready(function() {
    ...
    switch (window.searchType){
        ...
        case 'R':
            ...
            break;
        case 'O':
            oipaLink = window.oipaApiUrl + 'activities/?hierarchy=1&page_size=20&format=json&reporting_organisation_identifier='+window.ogd_code+'&fields=id,activity_dates,aggregations,activity_status,id,iati_identifier,url,title,reporting_organisation,activity_plus_child_aggregation,descriptions&activity_status='+$('#activity_status_states').val()+'&ordering='+$('.sort_results_type:first').val()+'&total_hierarchy_budget_gte='+budgetLowerBound+'&total_hierarchy_budget_lte='+budgetHigherBound+'&actual_start_date_gte='+$('#date_lower_bound').val()+'&planned_end_date_lte='+$('#date_higher_bound').val()+'&sector='+$('#selected_sectors').val()+'&document_link_category='+$('#selected_document_type').val() +'&participating_organisation='+$('#selected_implementingOrg_type').val();
            break;
    }
    ...
    function refreshLHSButtons(){
        ...
        $( "#amount" ).html('<label class="visually-hidden" for="budget_slider_start">Enter budget lower end value for search</label><input id="budget_slider_start" style="color: black; width: 35%; font-size: 0.8em;" value="£'+addCommas($( "#slider-vertical" ).slider( "values", 0 ))+'"> - <label class="visually-hidden" for="budget_slider_end">Enter budget higher end value for search</label><input id="budget_slider_end" value="£'+addCommas($( "#slider-vertical" ).slider( "values", 1 ))+'" style="color: black; width: 35%; font-size: 0.8em;"><button id="budget_slider_update" style="font-size: 0.8em; padding: 0px; margin: 0px 0px 0px 5px; width: 13%; height: 25px;">Go</button>');
        $('#date-range').html('<label class="visually-hidden" for="date_slider_start">Enter start date for date range search</label><input id="date_slider_start" style="color: black; width: 35%; font-size: 0.8em;" value=""> - <label class="visually-hidden" for="date_slider_end">Enter end date for date range search</label><input id="date_slider_end" value="" style="color: black; width: 35%; font-size: 0.8em;"><button id="date_slider_update" style="font-size: 0.8em; padding: 0px; margin: 0px 0px 0px 5px; width: 13%; height: 25px;">Go</button>');
    ...
    function populateSectorFilters(sectorList){
        if(sectorList.length > 0){
            ...
            tempSectors = '<div class="proj-filter-exp-collapse-sign proj-filter-exp-collapse-sign-up"></div><span class="proj-filter-exp-collapse-text" style="cursor:pointer"><h3>Sectors</h3></span><div class="mContent"><fieldset class="no-style"><legend>Sectors</legend><ul style="display: block; margin: 5px;">' + tempSectors + '</ul></fieldset></div><input type="hidden" id="selected_sectors" value=""  />';
    ...
    function populateDocumentTypeFilters(documentTypes){
        if(documentTypes.length > 0){
            ...
            tempDocuments = '<div class="proj-filter-exp-collapse-sign proj-filter-exp-collapse-sign-up"></div><span class="proj-filter-exp-collapse-text" style="cursor:pointer"><h3>Document Type</h3></span><div class="mContent"><fieldset class="no-style"><legend>Document Type</legend><ul style="display: block; margin: 5px;">' + tempDocuments + '</ul></fieldset></div><input type="hidden" id="selected_document_type" value=""  />';
    ...
    function populateImplOrgFilters(orgList){
        if(orgList.length > 0){
            ...
            tempOrgs = '<div class="proj-filter-exp-collapse-sign proj-filter-exp-collapse-sign-up"></div><span class="proj-filter-exp-collapse-text" style="cursor:pointer"><h3 style="margin-left: 5px; padding-bottom: 5px">Implementing Organisations</h3></span><div class="mContent"><fieldset class="no-style"><legend>Implementing Organisations</legend><ul class="implenting-orgs implenting-orgs-container" style="overflow: hidden; display: none; margin: 5px;">' + tempOrgs + '</ul></fieldset></div><input type="hidden" id="selected_implementingOrg_type" value=""  /><div class="description-overlay" style="display: none; position: relative;top: 0px;bottom: 0px;width: 100%;text-align: center;background-image: -webkit-gradient(linear,left top,left bottom,color-stop(0, rgba(255, 255, 255, 0)),color-stop(1, rgb(255, 255, 255)));padding: 20px 0 0 0;color: blue;text-decoration: blink;font-weight: bold;background-image: -moz-linear-gradient(top, transparent, white)"><div class="description-expander" style="display: none; text-align: center; color: #2e3191; cursor: pointer">Expand</div></div>';
    ...
    function populateLocRegionFilters(regionList){
        if(regionList.length > 0){
            ...
            tempRegionLocations = '<div class="proj-filter-exp-collapse-sign proj-filter-exp-collapse-sign-up"></div><span class="proj-filter-exp-collapse-text" style="cursor:pointer"><h4>Regions</h4></span><div class="mContent"><fieldset class="no-style"><legend>Regions</legend><ul style="display: block;margin: 5px;">' + tempRegionLocations + '</ul></fieldset></div><input type="hidden" id="locationRegionFilterStates" value=""  />';
    ...
    $('.activity_status').click(function(){
        ...
        case 'R':
            ...
            break;
        case 'O':
            oipaLink2 = window.oipaApiUrl + 'activities/?hierarchy=1&page_size=20&format=json&reporting_organisation_identifier='+window.ogd_code+'&fields=id,activity_dates,aggregations,activity_status,id,iati_identifier,url,title,reporting_organisation,activity_plus_child_aggregation,descriptions&activity_status='+$('#activity_status_states').val()+'&ordering=-activity_plus_child_budget_value&total_hierarchy_budget_gte=&total_hierarchy_budget_lte=&actual_start_date_gte=&planned_end_date_lte=&sector=&document_link_category=&participating_organisation=';
            break;
    }
        refreshOipaLink(window.searchType,0);
